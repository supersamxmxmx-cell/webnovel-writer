...
export COMMAND_ROOT="${PLUGIN_ROOT}/commands/webnovel-query"
...
| 查询类型 | Reference | 实际路径 |
|---------|-----------|---------|
| 数据流 / 优先级 | 数据流规范 | `${COMMAND_ROOT}/references/system-data-flow.md` |
| 伏笔分析 | 伏笔分析 | `${COMMAND_ROOT}/references/advanced/foreshadowing.md` |
| 节奏分析 | Strand 模式 | `${COMMAND_ROOT}/../../references/shared/strand-weave-pattern.md` |
| 格式查询 | 标签规范 | `${COMMAND_ROOT}/references/tag-specification.md` |
...
```bash
# 角色历史状态：某实体在指定章节时的状态
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" knowledge query-entity-state --entity "{entity_id}" --at-chapter {N}

# 实体关系：某实体在指定章节时的所有关系
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" knowledge query-relationships --entity "{entity_id}" --at-chapter {N}

# 世界规则
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" memory-contract query-rules --chapter {chapter_num}

# 伏笔 / open loop
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" memory-contract get-open-loops

# 仅综合 / 复杂查询：需要时间线 + 长期记忆联合时才用
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" memory-contract load-context --chapter {chapter_num}
```
