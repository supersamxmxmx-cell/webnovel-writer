...
export COMMAND_ROOT="${PLUGIN_ROOT}/commands/webnovel-init"
...
| 命名 | 开始命名 | `${PYTHON} "${SCRIPTS_DIR}/reference_search.py" --skill init --table 命名规则 --query "{命名对象} {题材}" --genre {题材}` |
...
- `Agent`：拆分并行子任务；Step 1.5 用户选择参考书拆解作灵感来源时调用 `deconstruction-agent` (see `${PLUGIN_ROOT}/agents/deconstruction-agent.md`)。
...
if [ -z "${PLUGIN_ROOT}" ] || [ ! -d "${PLUGIN_ROOT}/scripts" ]; then
  echo "ERROR: 未设置 PLUGIN_ROOT 或缺少目录: ${PLUGIN_ROOT}/scripts" >&2
  exit 1
fi
export SCRIPTS_DIR="${PLUGIN_ROOT}/scripts"
```
...
当用户选择参考作品拆书且提供文本路径或章节摘录时，必须使用 `Agent` 工具调用 `deconstruction-agent` (see `${PLUGIN_ROOT}/agents/deconstruction-agent.md`)，不得由 init 主流程口头替代拆解结果。

```text
Use the Agent tool (subagent_type="general-purpose") to run a deconstruction-agent task. Read `${PLUGIN_ROOT}/agents/deconstruction-agent.md` for the agent's full instructions, then pass those instructions plus the task parameters below as the prompt.
...
```bash
PROJECT_SLUG="$(${PYTHON} -c "import re,sys; title=sys.argv[1].strip(); slug=re.sub(r'[\\\\/:*?\"<>|]+','',title); slug=re.sub(r'\\s+','-',slug).strip('-'); print(('proj-' + slug) if (not slug or slug.startswith('.')) else slug)" "{title}")"
...
```bash
GENRE="$(${PYTHON} -c "import json,os; root=os.environ['PROJECT_ROOT']; s=json.load(open(root + '/.webnovel/state.json',encoding='utf-8')); pi=s.get('project_info',{}); print(pi.get('genre') or s.get('project',{}).get('genre',''))")"
