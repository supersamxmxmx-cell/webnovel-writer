...
export COMMAND_ROOT="${PLUGIN_ROOT}/commands/webnovel-write"
...
```bash
${PYTHON} "${SCRIPTS_DIR}/reference_search.py" --skill write --table {表名} --query "{关键词}" --genre {题材}
```
...
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${WORKSPACE_ROOT}" preflight
...
```bash
GENRE="$(${PYTHON} -c "import json,sys; s=json.load(open('${PROJECT_ROOT}/.webnovel/state.json',encoding='utf-8')); pi=s.get('project_info',{}); print(pi.get('genre') or s.get('project',{}).get('genre',''))")"
...
Use the Agent tool (subagent_type="general-purpose") to run a context-agent task. Read `${PLUGIN_ROOT}/agents/context-agent.md` for the agent's full instructions, then pass those instructions plus the task parameters below as the prompt.
...
Use the Agent tool (subagent_type="general-purpose") to run a reviewer task. Read `${PLUGIN_ROOT}/agents/reviewer.md` for the agent's full instructions, then pass those instructions plus the task parameters below as the prompt.
...
Use the Agent tool (subagent_type="general-purpose") to run a data-agent task. Read `${PLUGIN_ROOT}/agents/data-agent.md` for the agent's full instructions, then pass those instructions plus the task parameters below as the prompt.
...
```bash
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" chapter-commit \
  --chapter {chapter_num} \
...
```bash
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" backup \
  --chapter {chapter_num} \
  --chapter-title "{title}"
```
...
```bash
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" run-ledger write-resume \
  --chapter {chapter_num} \
  --mode "{mode}" \
  --format json
```
