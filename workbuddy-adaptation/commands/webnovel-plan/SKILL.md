...
export COMMAND_ROOT="${PLUGIN_ROOT}/commands/webnovel-plan"
...
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" placeholder-scan --format text
```
...
| 触发 | 读取方式 | 文件 |
|------|---------|------|
| Step 4 | 全文 | `${COMMAND_ROOT}/../../templates/output/大纲-卷节拍表.md` |
| Step 5 | 全文 | `${COMMAND_ROOT}/../../templates/output/大纲-卷时间线.md` |
| Step 6 always | 区段 | `${COMMAND_ROOT}/../../references/genre-profiles.md`（仅当前 genre 的 `### 2.x` 段） |
| Step 6 always | 全文 | `${COMMAND_ROOT}/../../references/shared/strand-weave-pattern.md` |
| 章纲拆分 always | 区段 | `${COMMAND_ROOT}/../../references/outlining/plot-signal-vs-spoiler.md` |
| Step 6 需要爽点 | 区段 | `${COMMAND_ROOT}/../../references/shared/cool-points-guide.md` |
| Step 6/7 需要冲突 | 区段 | `${COMMAND_ROOT}/references/outlining/conflict-design.md` |
| Step 6/7 特定节奏 | 区段 | `${COMMAND_ROOT}/references/outlining/genre-volume-pacing.md` |
| Step 7 追读力分析 | 区段 | `${COMMAND_ROOT}/../../references/reading-power-taxonomy.md` |
| Step 7 章纲细化 + 节点规范 | 区段 | `${COMMAND_ROOT}/references/outlining/chapter-planning.md` |

CSV 创作参考用检索读，不 `cat` 整表：

```bash
${PYTHON} "${SCRIPTS_DIR}/reference_search.py" --skill plan --table 爽点与节奏 --query "{卷级核心冲突}" --genre "${GENRE}"
${PYTHON} "${SCRIPTS_DIR}/reference_search.py" --skill plan --table 桥段套路 --query "{卷级核心冲突}" --genre "${GENRE}"
${PYTHON} "${SCRIPTS_DIR}/reference_search.py" --skill plan --table 命名规则 --query "角色命名" --genre "${GENRE}"
```
...
GENRE="$(${PYTHON} -c "import json; s=json.load(open('${PROJECT_ROOT}/.webnovel/state.json',encoding='utf-8')); pi=s.get('project_info',{}); print(pi.get('genre') or s.get('project',{}).get('genre',''))")"
```
...
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" knowledge query-entity-state --entity "{protagonist_id}" --at-chapter {上一卷最后章}
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" knowledge query-relationships --entity "{protagonist_id}" --at-chapter {上一卷最后章}
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" memory-contract get-open-loops
```
...
加载模板 `${COMMAND_ROOT}/../../templates/output/大纲-卷节拍表.md`。
...
加载模板 `${COMMAND_ROOT}/../../templates/output/大纲-卷时间线.md`。
...
必读 `${COMMAND_ROOT}/../../references/genre-profiles.md` 与 `${COMMAND_ROOT}/../../references/shared/strand-weave-pattern.md`；按需读取爽点 / 冲突 / 节奏 reference（见读取策略表）。
...
按需读取 `${COMMAND_ROOT}/../../references/reading-power-taxonomy.md` 与 `${COMMAND_ROOT}/references/outlining/chapter-planning.md`。

每章必须包含：目标、阻力、代价、时间锚点、章内时间跨度、与上章时间差、倒计时状态、爽点、Strand、反派层级、视角/主角、关键实体、本章变化、章末未闭合问题、钩子，以及结构化节点 `CBN`、`CPNs`、`CEN`、`必须覆盖节点`、`本章禁区`。

#### 结构化节点

节点格式统一为 `主体 | 动作/变化 | 对象/结果`（写作执行骨架，不追求严格语法 SVO）。完整格式说明、字段细则与示例见 `${COMMAND_ROOT}/references/outlining/chapter-planning.md` 的「结构化节点规范」，按需区段读，不在本文件内联。
...
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" story-system "${CHAPTER_GOAL}" \
...
```bash
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" user-report \
  --stage plan \
  --volume {volume_id} \
  --format text
```
