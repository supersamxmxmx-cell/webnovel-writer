...
```bash
# 查询角色当前状态
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" state get-entity --id "{entity_id}"
# 查询最近状态变更
${PYTHON} "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" index get-state-changes --limit 20
```
## 3. 输入
- `chapter`：章节号
- `chapter_file`：正文文件路径
